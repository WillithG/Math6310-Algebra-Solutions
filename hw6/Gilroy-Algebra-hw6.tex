...
$X \times Y := \{(x,y) : x \in X \qquad y \in Y\}$. Define a
$G$-action on $X \times Y$ by $\sigma(x,y) = (\sigma x, \sigma y)$ for
$\sigma \in G$ where $\sigma x, \sigma y$ are given by the $G$-action
structure on $X, Y$. 
We verify that this is indeed a $G$-action on $X \times Y$. 
Note that if $1 \in G$ is the identity then we have $1(x,y) = (1x, 1y)
...
But recall that $(X \times Y, \pi_X, \pi_Y)$ satisfies the universal
...
since it satisfies the universal property in $\Set$. 
...
That is, $f : Z \to X \times Y$ is a morphism in $\Set_G$ and it
follows that its the unique morphism which makes the following diagram
...
\wg{Do empty products count as a finite product?}
