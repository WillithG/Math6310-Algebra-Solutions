...
Let us first describe the maps for some fixed ring
$R$. 
...
and $\bar f(\bar y) = r_2$.
Conversely, a map in $\tilde f \in \Z$ is determined exactly by 
homomorphisms $f: \Z[x,y] \to R$ satisfying $f(I) = 0$. 
Moreover, these homomorphisms are determined by the images 
$f(x), f(y)$.
\wg{So, what we're really saying is that
...
we have\[
	f(y)^3 - f(x)^3 - f(x)^2 
		...
		= 0
\]
By definition of $f$. Hence, this proposed function is well-defined.
\wg{What next?}
\wg{check that the function is an isomorphism.
check naturality. clean up the logic of the above statements.}
