...
\item 
I will go a bit faster through this argument since many of the details
follow similar logic to the above part.

I claim that $G: \textbf{Ring} \to \Set$ is representable functor,
represented by the ring $\Z[x, x\inv]$. 
We must show that there exists a natural isomorphism $Hom_\Ring(\Z[x,
x\inv], -) \to G$. 
First note that the maps $f \in Hom_\Ring(\Z[x, x\inv])$ are still
determined by their action on $x \in \Z[x, x\inv]$.
But now, since $1 = f(1) = f(x x\inv) = f(x)f(x\inv)$ any ring
homomorphism $\Z[x, x\inv] \to R$ must send $x$ to a unit in $R$. 
That is, the maps $f \in Hom_\Ring(\Z[x,x\inv], R)$ are exactly
determined by the image on $x$, with the restriction that the image of
$x$ must be a unit in $R$. 

Now we construct a natural transformation $Hom_\Ring(\Z[x,x\inv], -)
\to G$. Given $R \in \Ring$ we assign the function $f_R :
Hom(\Z[x,x\inv], R) \to R^\times$ given by evaluating the input map at
$x$, $f_R(f) := f(x)$. 
We note that under this definition $f_R$ is a well defined map into
$R^\times$ becuase, as we noted above, the image of $x$ under a ring
homomorphism $\Z[x, x\inv]$ must be a unit. 
Next, notice that each $f_R$ is actually a bijection since it has
inverse $r \in R^\times$ maps to the homomorphism where $f(x) = r$,
using similar reasoning to the discussion for $F$ above. 

Lastly, we need to show that with these definitions, and given a ring
morphism $g: R \to S$, the following
diagram commutes \[
\begin{tikzcd}
{Hom(\Z[x,x\inv], R)} \arrow[r, "f_R"] \arrow[d, "g'"'] & R \arrow[d, "g"] \\
{Hom(\Z[x,x\inv], S)} \arrow[r, "f_S"]                  & S               
\end{tikzcd}
\]
Following the top side we have \[
	f \mapsto f(x) \mapsto (g \circ f)(x),
\]
meanwhile following the bottom side gives \[
	f \mapsto g \circ f \mapsto (g \circ f)(x).
\]
And so this diagram commutes for any ring homomorphism.
We have found a natural isomorphism from $Hom_\Ring(\Z[x,x\inv], -)
\to G$ and so $G$ is a representable functor, represented by the ring
$\Z[x,x\inv]$. 

\item 
We claim that $H$ can be represented by the ring $\Z[x,y]$. 
\wg{Notes to self: maps out of this ring are determined exactly by
the images of $x,y$. similar reasoning to showing $\Z[x]$ represented
$F$ then ensues}.

\item
We claim that $K$ is can be represented by the ring $\tilde Z = \Z[x,y]/((y^2 -
x^3 - x^2))$. 
To prove this claim we need to find a natural bijection of sets 
$Hom(\tilde Z, R) \xrightarrow{\sim} K(R)$ for each $R \in \Ring$.

\wg{we definitely want to read over this paragraph again.}
Let us first describe the maps $\Z[x,y] / I \to R$ 
for some fixed ring $R$. 
Given two elements $r_1, r_2 \in R$ there is a unique ring
homomorphism $f: \Z[x,y] \to R$ with $f(x) = r_1$ and $f(y) = r_2$.
If $f$ further satisfies $f(I) = 0$ then, by the universal property of
quotients, there exist a unique ring morphism $\bar f: \tilde Z \to R$ 
making the following diagram commute
\[
\begin{tikzcd}
{Z[x,y]} \arrow[r, "f"] \arrow[d, "\pi"'] & R \\
{Z[x,y]/I} \arrow[ru, "\bar f"']          &  
\end{tikzcd}
\]
That is, $\bar f: \tilde Z \to R$ satisfying $\bar f(\bar x) = r_1$
and $\bar f(\bar y) = r_2$.
Conversely, a map in $\tilde f \in \Z[x,y]/I$ is determined exactly by 
homomorphisms $f: \Z[x,y] \to R$ satisfying $f(I) = 0$. 
Suppose we have a ring morphism $\tilde g: \Z[x,y]/I \to R$ then
we have a ring homomorphism $(\pi \circ g): \Z[x,y] \to R$ satisfying
$(\pi \circ g)(I) = 0$. The universal property of quotients then
recovering the map $g$.
$Hom_\Ring(\Z[x,y]/I, R) \xrightarrow{\sim}
\{f \in Hom_\Ring(\Z[x,y], R): f(I) = 0\}$
And so a homomorphism $\Z[x.y]/I \to R$ is determined by the images 
of $x,y$.

Now, still with a fixed ring $R$,
consider the function $Hom_\Ring(\tilde Z, R) \to K(R)$ defined by 
$f \mapsto (f(x), f(y))$.
We verify that this is a well-defined function to $K(R)$. 
...
we have\[
	f(y)^3 - f(x)^3 - f(x)^2 
		...
		= 0
\]
By definition of $f$. And so indee the pair $(f(x), f(y))$ is an
element in $K(R)$. 
Moreover, this function is in fact a bijection. Given $r_1, r_2 \in
K(R)$ we have a unique function $f: \Z[x,y] \to R$ determined by 
$f(x) = r_1$ and $f(y) = r_2$. Furthermore, by definition of $r_1,
r_2$ we further have \[
	f(y^2 - x^3 + x) = r_2^2 - r_1^3 + r_1 = 0.
\]
That is, $f(I) = 0$ and so $f$ has a unique extension to the quotient
$\Z[x,y]/I \to R$. 
That is, the components $\alpha_R$ are isomorphisms for all $R \in
\Ring$.

Lastly, given a ring homomorphism $f: R \to S$,
we need to verify that our definition for the components 
$\alpha_R$ makes the following diagram commute \[
\begin{tikzcd}
{Hom_\Ring(\Z[x,y]/I, R)} \arrow[r, "\alpha_R"] \arrow[d] & K(R) \arrow[d] \\
{Hom_\Ring(\Z[x,y]/I, S)} \arrow[r, "\alpha_S"]           & K(S)          
\end{tikzcd}
\]
Before, we can check commutivity of this diagram, we should be clear
about the vertical maps.
Given $h: \Z[x,y]/I \to R$ the image of $f$ under the functor $Hom_\Ring(\Z[x,y]/I, -)$ sends
$h$ to $f \circ h$. Moreover, given $(r_1, r_2) \in KR$, the image of
$f$ under the functor $K$ sends $(r_1, r_2)$ to $(f(r_1), f(r_2))$.
See the solution to the previous question to see that this is the
correct image under $K$.

Now, for some fixed $R,S \in \Ring$ and $f: R\to S$, given $h \in
Hom(\Z[x,y]/I, R)$ following the top path gives \[
	h \mapsto (h(x),h(y)) \to ((f \circ h)(x), (f \circ h)(y)),
\]
whereas, following the bottom path gives \[
	h \mapsto f \circ h \mapsto ((f \circ h)(x), (f \circ h)(y)).
\]
And so we have found a natural bijection $Hom_\Ring(\Z[x,y]/I, R)
\xrightarrow{\sim} K(R)$ for each $R \in \Ring$. Hence the functor $K$
is represented by the ring $\Z[x,y]/(y^2 - x^3 + x)$. 
