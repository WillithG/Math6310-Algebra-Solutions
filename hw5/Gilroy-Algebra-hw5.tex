...
\problemlist{Algebra Homework \#5} 
...
Normal Form \[
	...
	\end{bmatrix}
\]
Note, since $M \leq R^n$ we have that $n \geq k$. 
But notice, when $A$ is in Smith normal form then we manifestly
have that the columns of $A$ are linearly independent. That is,
$A: R^m \to R^n$ is an injective transformation. 
The only way for this to be true is if $\sigma: R^k \to M$ is also an
injective transformation. But then $\sigma$ is a bijective $R$-linear
transformation, and so we have shown that $M \cong R^k$. In other
words, that $M$ is a free $R$-module.
...
\begin{solution}
\wg{note, only partially completed}
...
also. I think the lower degree cases are more straightforward}
...
\begin{solution}
Using the result of problem $4$, we can find the rational canonical
form of this matrix by finding its characteristic and minimal
polynomial. By inspecting the characteristic and minimal polynomials
of a $3 \times 3$ matrix in rational canonical form, we will have that
the characteristic polynomial $P_\alpha(\lambda) = f_1\cdots f_m$
where $f_i$ are the invariant factors of $A$. Recall that these
satisfy $f_1 \vert f_2 \vert \cdots \vert f_m$.
Moreover, the minimal
polynomial $m_\alpha(\lambda) = f_m$.
Note, that alternatively, we can find the rational canonial form of
$A$ by putting $A - xI$ into Smith Normal Form.

We use $P_\alpha(\lambda) = det(A - \lambda I)$ to compute the
characteristic polynomial. After using some row and column operations
we find that \[
	P_\alpha(\lambda) = (\lambda+1)(\lambda-2)^2.
\]
Recall that the minimal polynomial is the smallest degree polynomial
$f$ such that $f(A) = 0$. By the Cayley-Hamilton theorem we have that
$P_\alpha(A) = 0$ and so we can find $m_\alpha(A)$ by enumerating each
of the polynomials which divide $P_\alpha$, by combining each of the
factors that we have above. Notice that no degree $1$ divisor of
$P_\alpha$ vanishes on $A$, and so $M_\alpha$ must be degree two or
greater. Inspecting the possibilities, one will find that
$(\lambda+1)(\lambda-2)$ vanishes on $\lambda= A$. And so \[
	m_\alpha(\lambda) = (\lambda+1)(\lambda-2).	
\]

Then we have $f_m(x) = (x+1)(x-2)$. By enumerating the
divisors of $f_m$ and by recalling that $P_\alpha(\lambda) = f_1
\cdots f_m = (\lambda+1)(\lambda-2)^2$ we find that we must in fact
have $m = 2$ and $f_1 = \lambda-2$. To summarize, our invariant
factors are
\[
	f_1(x) = x -2 \qquad f_2(x) = (x+1)(x-2) = x^2 - x -2
\]
These factors have companion matrices \[
	C_{f_1} = \begin{bmatrix}
		2	
	\end{bmatrix}
	\qquad
	C_{f_2} = \begin{bmatrix}
		0 & 2 \\
		1 & 1
	\end{bmatrix}
\]
And overall the rational canonical form of $A$ is \[
	A \sim \begin{bmatrix}
		2 & & \\
		  & 0 & 2 \\
		  & 1 & 1
...
Now, we can determine the eigenvalues of $C_f^2$ from first
principles. If an $n\times n$ matrix $A$ has $\lambda$ as an
eigenvalue that means there's some $v \in R^n$ such that $Av = \lambda
v$. It immediately follows that \[
A^2 v = A(A v) = A (\lambda v) = \lambda(A v) = \lambda^2 v.
\]
That is, we have $v \in R^n$ such that $A^2 v = \lambda^2 v$ and so
$\lambda^2$ is an eigenvalue of $A$. 
In particular then, the eigenvalues of $C_f$ are $\{\alpha_i^2 \}$. 

Using similar reasoning, since $\alpha_i$ is eigenvalue of $C_f$,
there exists some $v_{\alpha_i} \in K^n$ such that $C_f v = \alpha_i v$. Now
consider \[
(C_f^3 + 2C_f + 3I_n)v_{\alpha_i} = (\alpha_i^3 + 2\alpha_i + 3)
v_{\alpha_i}, 
\]
This holds for all $\alpha_i$ and so the eigenvalues of $C_f^3 + 2C_f
+ 3I_n$ are the values in $\{\alpha_i^3 + 2\alpha_i + 3\}$

Let $g(x) = x^3 + 4x + 5 \in K[x]$. From the above, we have that $C_g$
has eigenvalues the roots of $g$ and $C_g^2$ are eigenvalues the the
squares of the roots of $g$. We have \[
	C_g^2 = 
	\begin{bmatrix}
		0 & 0 & -5 \\
		1 & 0 & -4 \\
	...
	\end{bmatrix}.
\]
Now if we can write $C_g^2$ in the usual companion matrix form, we
could immediately read the desired monic polynomial from the entires.
Recall that change of basis on the domain and codomain of a matrix
does not change its characteristic polynomial, and so also preserves
its eigenvalues. That is, we may conjugate $C_g^2$ by invertible
matrices to change its form.
Let us put $C_g^2$ into rational canonical form. We have the
characteristic polynomial of $C_g^2$ is given by  \[
\text{char}_{C_g^2}(\lambda) = \lambda^3 + 8\lambda^2 + 16\lambda - 25 
= (\lambda-1)(\lambda^2 +9\lambda + 25).
\]
Recall also that the this characteristic polynomial is equal to the
product of the invariant factors of $C_g^2$. By plugging $C_g^2$ into each of
the factors, we find that the minimal polynomial is equal to the
characteristic polynomial. Then we have a single invariant factor for
$C_g^2$ given by the characteristic polynoimal above and then we can
write $C_g^2$ in rational canonical form as \[
	C_g^2 \sim \begin{bmatrix}
		0 & 0 & 25 \\
		1 & 0 & -16 \\
		0 & 1 & -8
	\end{bmatrix},
\]
	and so we can deduce that the following polynomial, which has
	roots eigenvalues of $C_g^2$ which are the squares of the roots of
...
\begin{solution}
\wg{prove the first part}

Recall that matrices which are similar have the same characteristic
polynomials. That is, $A$ has the same characteristic polynomial to 
the rational canonical form matrix of $A$, call it $B$. Recall that
$B$ is block diagonal with its blocks being companion matrices of the
invariant factors of $A$. Each of the blocks $C_{f_i}$ have characteristic
polynomial $f_i$ and so we have recovered the fact $P_\alpha(\lambda)
= f_1 \cdots f_m$. Write \[
	B = 
	\begin{bmatrix}
		C_{f_1} & & & \\
	...
	\end{bmatrix}. 
\]
Then, by the first part of the problem, we have \[
	f_1(B) = 
	\begin{bmatrix}
		\mathbf{0} & & & \\
		& C_{f_2} & & \\
	...
	\end{bmatrix}.
\]
And then it follows that $p(B) = f_1(B)\cdots f_m(B) = 0$.
	Now, since $A \sim B$ we have that $A = CBC\inv$ for some
	invertible square matrix $C$. Then we have \[
		p(A) = p(CBC\inv) = C p(B) C\inv = 0,
	\]
	and so, $A$ vanishes on its characteristic polynomial.
	\wg{by inspecting a few cases, I'm pretty sure the second equality
	is true, but we should probably prove it. I think it should be 
	somewhat easy to do by writing $p = \sum a_i x^i$.}
\end{solution}
